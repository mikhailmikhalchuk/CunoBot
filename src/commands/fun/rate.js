...
module.exports = {
    ...
    func: async (message, args) => {
        ...
        if (args == "" || args == undefined) return message.channel.send(global.Functions.BasicEmbed(("error"), "Please provide a message to rate."))
            ...
            if (message.author.id == "287372868814372885") {
                ...
                if (cachedRatings[message.member.id] != undefined) return message.reply(`I'd give you a ${cachedRatings[message.member.id]}/10`)
                else {
            ...
            if (cachedRatings[args.join(" ")] != undefined) return message.reply(`I'd give ${args.join(" ")} a ${cachedRatings[args.join(" ")]}/10`)
            else {
                var random = Math.floor(Math.random() * 11)
